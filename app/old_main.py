...
# for v2
# app.include_router(v2_router, prefix="/v2", tags=["v1-videos"])
...
async def save_file_to_disk(file: UploadFile, temp_file_path: str):
    async with aiofiles.open(temp_file_path, "wb") as temp_file:
        # Stream the file in chunks
        while chunk := await file.read(1024):  # Read 1024 bytes at a time
            await temp_file.write(chunk)  # Write the chunk to the file
...
@app.post("/read-file")
async def read_file(file: UploadFile, background_tasks: BackgroundTasks):

    filename = file.filename
    file_content = await file.read()

    background_tasks.add_task(save_uploaded_file, file_content, filename)
    
    # background_tasks.add_task(process_send_email)
    return {"You will get email soon"}

def process_send_email():
    time.sleep(5)
    print("email is sent now.")
